...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    //MYSQL
    const usuarioMy: any = await Usuario.findOne({ where: { username },
      include: [
        {
          model: Role,
          as: "role",
    ...
    //MONGODB
    const usuarioMo: any = await UsuarioM.findOne({ username })
    ...
    if (!usuarioMy || !usuarioMo) {
      return res.status(400).json({
        msg: "Usuario / Password no son correctos - username(borrar luego)",
      });
    }
    ...
    if (usuarioMy.estado === false || usuarioMo.estado === false) {
      return res.status(400).json({
        msg: "Usuario / Password no son correctos - estado:false(borrar luego)",
      });
    }
    ...
    if (!validPassword || !validPasswordMo) {
      return res.status(400).json({
        msg: "Usuario / Password no son correctos - password(borrar luego)",
      });
    }
...
export const revalidarToken = async (req: any, res: Response) => {
  ...
  if (!usuarioMy) {
    usuarioToken = {
      ...
      username: usuarioMo.username,
      nombre: usuarioMo.nombre,
      role: usuarioMo.role.rol_nombre,
      correo: usuarioMo.correo,
      apellido: usuarioMo.apellido,
    };
  } else {
    usuarioToken = {
      id: usuarioMy.id,
      ...
      correo: usuarioMy.correo,
      apellido: usuarioMy.apellido,
    };
  }
